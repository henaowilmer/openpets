...
assert.deepEqual(parseReactArgs(["success", "--backend", "termux"]), { reaction: "success", backend: "termux" });
assert.throws(() => parseReactArgs([]));
assert.throws(() => parseReactArgs(["bad"]));
assert.throws(() => parseReactArgs(["success", "--backend", "invalid"]));
...
assert.deepEqual(parseSayArgs(["Tests", "passed", "--backend", "termux"]), { message: "Tests passed", reaction: undefined, backend: "termux" });
